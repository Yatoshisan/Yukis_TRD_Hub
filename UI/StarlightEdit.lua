...
local function Hide(Interface, JustHide: boolean?, Notify: boolean?, Bind: string?)
	...
	-- hide popups (only when hiding the main ScreenGui — NOT for individual frames like
	-- notifications, otherwise the simulated click steals focus from the chat textbox)
	if Interface.ClassName == "ScreenGui" then
...
local function Unhide(Interface)
	...
	for i, v in pairs(Interface:GetDescendants()) do
		...
		then
			pcall(function()
				local saved = TransparencyValues[Interface.Name][v:GetAttribute("InstanceID")]
				if not saved then return end

				if v.ClassName == "Frame" and saved.BackgroundTransparency ~= nil then
					v.BackgroundTransparency = saved.BackgroundTransparency
				end

				if (v.ClassName == "TextLabel" or v.ClassName == "TextBox" or v.ClassName == "TextButton")
					and saved.BackgroundTransparency ~= nil
					and saved.TextTransparency ~= nil
				then
					v.BackgroundTransparency = saved.BackgroundTransparency
					v.TextTransparency = saved.TextTransparency
				end

				if (v.ClassName == "ImageLabel" or v.ClassName == "ImageButton")
					and saved.BackgroundTransparency ~= nil
					and saved.ImageTransparency ~= nil
				then
					v.BackgroundTransparency = saved.BackgroundTransparency
					v.ImageTransparency = saved.ImageTransparency
				end

				if (v.ClassName == "UIStroke" or v.ClassName == "UIGradient")
					and saved.Transparency ~= nil
				then
	...
	pcall(function()
		if Interface.ClassName ~= "ScreenGui" then
			local saved = TransparencyValues[Interface.Name][Interface:GetAttribute("InstanceID")]
			if not saved then return end

			if Interface.ClassName == "Frame" and saved.BackgroundTransparency ~= nil then
				Interface.BackgroundTransparency = saved.BackgroundTransparency
			end

			if (Interface.ClassName == "TextLabel" or Interface.ClassName == "TextBox" or Interface.ClassName == "TextButton")
				and saved.BackgroundTransparency ~= nil
				and saved.TextTransparency ~= nil
			then
				Interface.BackgroundTransparency = saved.BackgroundTransparency
				Interface.TextTransparency = saved.TextTransparency
			end

			if (Interface.ClassName == "ImageLabel" or Interface.ClassName == "ImageButton")
				and saved.BackgroundTransparency ~= nil
				and saved.ImageTransparency ~= nil
			then
				Interface.BackgroundTransparency = saved.BackgroundTransparency
				Interface.ImageTransparency = saved.ImageTransparency
			end

			if (Interface.ClassName == "UIStroke" or Interface.ClassName == "UIGradient")
				and saved.Transparency ~= nil
			then
				Interface.Transparency = saved.Transparency
			end
		end
	end)
...
function Starlight:Notification(data)
	...
	task.spawn(function()
		...
		newNotification.LayoutOrder = #StarlightUI.Notifications:GetChildren() -- set before parenting so count excludes self
		...
		local notifTimeConn
		notifTimeConn = RunService.Heartbeat:Connect(function()
		...
		if data.Duration < 0 then
			-- infinite notification — disconnect the timestamp ticker since it will never expire
	...
	function Starlight.Window:PromptDialog(ModalSettings)
		...
		if Modal.Values.Type == 1 then
			...
			for _, Action in pairs(Modal.Values.Actions) do
				...
				if Action.Enter == nil then Action.Enter = true end
				...
				ActionInput.PART_Input:GetPropertyChangedSignal("Text"):Connect(function()
					if Action.MaxCharacters > 0 then
						if #ActionInput.PART_Input.Text > Action.MaxCharacters then
	...
	function Starlight.Window:CreateHomeTab(TabSettings)
		...
		if TabSettings.Backdrop then
			if TabSettings.Backdrop == 0 then
				...
				Tab.Instances.Page.ImageBackdrop.Visible = true
			end
		else
		...
		task.spawn(function()
			connections.__homeTabTime = RunService.Heartbeat:Connect(function()
				local t = os.date("*t")
				local hour = t.hour
		...
		local function getPing()
			return math.round((isStudio and Players.LocalPlayer:GetNetworkPing() or StatsService.PerformanceStats.Ping:GetValue()) * 1000)
		end
		...
		function TabSection:CreateTab(TabSettings, TabIndex)
			...
			function Tab:CreateGroupbox(GroupboxSettings, GroupIndex)
				...
				function Groupbox:CreateCheckbox(ElementSettings) -- will be merged with switch in next update via styles. adding a checkbox icon soon
					...
					ElementSettings.ImageSource = ElementSettings.ImageSource or "Material"
					if ElementSettings.InitialCallback == nil then ElementSettings.InitialCallback = true end
				...
				function Groupbox:CreateDropdown(ElementSettings)
					...
					Element.Instances.Element.Icon.MouseButton1Click:Connect(function()
						...
						outsideClickConn = UserInputService.InputBegan:Connect(function(i, g)
							...
							Option.Interact.MouseButton1Click:Connect(function()
								...
								if ElementSettings.MultipleOptions then
									if table.find(ElementSettings.CurrentOption, v) then
										Table.Remove(ElementSettings.CurrentOption, v)
								...
								CB(Selected, function()
									if ElementSettings.MultipleOptions then
										if not ElementSettings.CurrentOption or type(ElementSettings.CurrentOption) ~= "table" then
				...
				function Groupbox:CreateBind(ElementSettings) -- will be merged with toggles and labels soon
					...
					if ElementSettings.SyncToggleState == nil then ElementSettings.SyncToggleState = true end
					...
					Element.Instance.Bind.FocusLost:Connect(function()
						...
						if Element.Instance.Bind.Text == nil or Element.Instance.Bind.Text == "" then
							Element.Instance.Bind.Text = ElementSettings.CurrentValue
						end
					end)
					...
					task.spawn(function()
						...
						Element.Instance.PART_Backdrop.PART_Input:GetPropertyChangedSignal("Text"):Connect(function()
							if Element.Values.MaxCharacters > 0 then
								if #Element.Instance.PART_Backdrop.PART_Input.Text > Element.Values.MaxCharacters then
									Element.Instance.PART_Backdrop.PART_Input.Text =
				...
				function Groupbox:CreateLabel(ElementSettings, Index)
					...
					function Element:AddColorPicker(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
										...
										if Input.Name == "Hex" then
											inputinstance.Text = NestedElement.Values.Transparency == nil
												...
												or string.format(
													...
													math.round(color.G * 255),
													math.round(color.B * 255),
													math.round((1 - NestedElement.Values.Transparency) * 255)
												)
										end
							...
							do
								...
								local color = Color3.fromHSV(h, s, v)
								local hex =
									string.format("#%02X%02X%02X", math.round(color.R * 255), math.round(color.G * 255), math.round(color.B * 255))
										...
										inputinstance.FocusLost:Connect(function()
											...
											then
												inputinstance.Text = NestedElement.Values.Transparency == nil
													and string.format(
														"#%02X%02X%02X",
														math.round(NestedElement.Values.CurrentValue.R * 255),
														math.round(NestedElement.Values.CurrentValue.G * 255),
														math.round(NestedElement.Values.CurrentValue.B * 255)
													)
													or string.format(
														"#%02X%02X%02X%02X",
														math.round(NestedElement.Values.CurrentValue.R * 255),
					...
					function Element:AddDropdown(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
							...
							NestedElement.Instances[1].MouseEnter:Connect(onHover)
							NestedElement.Instances[1].MouseLeave:Connect(leave)
							NestedElement.Instances[2].MouseEnter:Connect(onHover)
							...
							local function ToggleOption(option)
								...
								if option:GetAttribute("Active") == false then
									...
									if
										...
										and #NestedElement.Values.CurrentOption <= 1
									then
										return
									end
